...
本次使用 `scripted_agent_fixture_v1`，且文章不含图片。因此它不证明正式改写质量、真实 Agent 生成、图片公网托管、macOS Computer Use 或真实微信采集可用，项目整体 readiness 仍为 `core_validated`。

本次验收发生在版本化 Content API 更新之前，因此本页本身没有验证新响应的 version/ETag、带认证的 `manage=true` 读取、PATCH、软删除、恢复或 revisions。新增能力后来已通过独立真实验收，见 [版本化 Content API 真实验收](2026-07-17-versioned-content-api-live-acceptance.md)；两份证据的范围不能混用。
