...
class MarkerWindowIntakeTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.chat_path = self.root / "scripted-chat.json"
    ...
    def initialize_chat(
        self, initial_messages: list[dict[str, object]] | None = None
    ) -> dict[str, Any]:
        ...
        return cast(dict[str, Any], json.loads(initialized.stdout))

    def append_messages(
        self,
        messages: list[dict[str, object]],
        arrive_after_marker: list[dict[str, object]] | None = None,
    ) -> None:
        chat = json.loads(self.chat_path.read_text("utf-8"))
        chat["messages"].extend(messages)
        chat["arrive_after_next_marker"] = arrive_after_marker or []
        self.chat_path.write_text(
    ...
    def task_records(self, task_ids: list[str]) -> list[dict[str, Any]]:
        return [
            json.loads(
                (self.repository / "tasks" / task_id / "task.json").read_text(
                    "utf-8"
                )
            )
    ...
    def test_initialize_sets_a_baseline_and_run_excludes_messages_after_its_marker(
        ...
        baseline_marker_id = initialized["baseline_marker_id"]
        ...
        run = json.loads(
            (self.repository / "runs" / run_id / "run.json").read_text("utf-8")
        )
        ...
        self.assertEqual(
            final_chat["messages"][marker_indexes[-1] + 1]["message_id"],
            "late-header",
        )

        next_result = self.run_intake()
        next_raw = json.loads(
            (
                self.repository
                / "tasks"
    ...
    def test_reinitialize_refuses_to_replace_an_existing_baseline(self) -> None:
        ...
        self.append_messages(
            [
                header("pending-header", "author-pending"),
                article("pending-article", "不能被新基线跳过"),
            ]
        )
        ...
        self.assertEqual(markers[0]["marker_id"], initialized["baseline_marker_id"])
        result = self.run_intake()
        self.assertEqual(len(result["task_ids"]), 1)
    ...
    def test_run_accepts_minimal_and_multiline_headers_and_isolates_unknown_fields(
        ...
        records = self.task_records(result["task_ids"])
